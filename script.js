document.addEventListener("DOMContentLoaded", function () {
        ...
        reserveButton.addEventListener("click", function () {
            if (selectedSlot && selectedDate) {
                ...
                if (!soinPrincipal) {
                    ...
                    const url = `formulaire.html?soin=${encodeURIComponent(soinPrincipal)}&ldate=${encodeURIComponent(selectedDate)}&creneau=${encodeURIComponent(selectedTime)}&prix=${encodeURIComponent(prix)}&soinsSupplementaires=${soinsSupplementairesEncoded}`;
                    window.location.href = url;
                }
    ...
    // Fonction pour supprimer un soin supplémentaire
    document.addEventListener("click", function (event) {
        if (event.target.classList.contains("delete-soin")) {
            const soinId = event.target.dataset.soinId;
            removeSoinFromStorage(soinId);
            removeSoinFromURL(soinId);
            event.target.closest(".soin-item").remove();
        }
    });

    // Supprime le soin du local storage
    function removeSoinFromStorage(soinId) {
        let soins = JSON.parse(localStorage.getItem("soins")) || [];
        soins = soins.filter(soin => soin.id !== soinId);
        localStorage.setItem("soins", JSON.stringify(soins));
    }

    // Supprime le soin de l'URL
    function removeSoinFromURL(soinId) {
        const urlParams = new URLSearchParams(window.location.search);
        let soins = urlParams.get("soins") ? urlParams.get("soins").split(",") : [];
        soins = soins.filter(id => id !== soinId);
        if (soins.length > 0) {
            urlParams.set("soins", soins.join(","));
        } else {
            urlParams.delete("soins");
        }
